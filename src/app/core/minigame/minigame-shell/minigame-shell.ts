    ...
    <div class="minigame-shell">
      <div class="shell-hud">
        <div class="shell-hud__primary">
          ...
          @if (showTimer()) {
            <div class="shell-hud__timer" [style.color]="timerColor()" [class.shell-hud__timer--pulse]="timerPulse()">{{ timeRemaining() }}s</div>
          }
        </div>
        <div class="shell-hud__lives">
          @for (life of livesArray(); track $index) {
            <span class="shell-hud__life" [class.shell-hud__life--empty]="!life.filled"></span>
          }
        </div>
        @if (hintCount() > 0 || activeHintText()) {
          <div class="shell-hud__hint-container">
            <button class="shell-hud__hint" type="button" [disabled]="hintCount() === 0" (click)="requestHint.emit()" [attr.aria-label]="'Use hint, ' + hintCount() + ' remaining, costs ' + hintPenalty() + ' points'">
              Hint <span class="shell-hud__hint-badge">{{ hintCount() }}</span>
              @if (hintPenalty() > 0 && hintCount() > 0) { <span class="shell-hud__hint-cost">(-{{ hintPenalty() }} pts)</span> }
            </button>
            @if (activeHintText()) {
              <div class="shell-hud__hint-popover" role="status" aria-live="polite">{{ activeHintText() }}</div>
            }
          </div>
        }
        <button class="shell-hud__shortcuts-toggle" type="button" (click)="toggleShortcutHints()" aria-label="Toggle keyboard shortcuts">?</button>
        <button class="shell-hud__pause" type="button" (click)="onPauseClick()" aria-label="Pause"></button>
      </div>
      @if (showShortcutHints()) {
        <div class="shell-hud__shortcuts-panel" role="status" aria-live="polite">
          ...
          @for (shortcut of registeredShortcuts(); track shortcut.key) {
            <div class="shell-hud__shortcut-item"><kbd>{{ shortcut.key }}</kbd><span>{{ shortcut.label }}</span></div>
          }
      ...
      <div class="shell-content"><ng-content /></div>
      @if (showTutorial() && gameId() && tutorialSteps().length > 0) {
        <nx-minigame-tutorial [gameId]="gameId()!" [steps]="tutorialSteps()" (dismissed)="tutorialDismissed.emit()" />
      }
      @if (status() === paused) {
        <nx-pause-menu (resume)="onResumeClick()" (restart)="restartGame.emit()" (howToPlay)="howToPlay.emit()" (quit)="quit.emit()" />
      }
      @if (status() === won && result()) {
        <nx-level-results [result]="result()!" [previousBest]="previousBest()" [scoreBreakdown]="scoreBreakdown()" [nextLevelLocked]="nextLevelLocked()" (nextLevel)="nextLevel.emit()" (replay)="replay.emit()" (quit)="quit.emit()" />
      }
      @if (status() === lost) {
        <nx-level-failed [reason]="failureReason()" [score]="score()" [hintsAvailable]="hintsAvailable()" (retry)="retry.emit()" (useHint)="useHint.emit()" (quit)="quit.emit()" />
      }
    </div>
  `,
...
export class MinigameShellComponent {
  ...
  readonly failureReason = computed(() => {
    if (this.lives() <= 0) return '3 strikes';
    if (this.timerDuration() > 0 && this.timeRemaining() <= 0) return 'Time expired';
  ...
  readonly timerColor = computed(() => {
    ...
    if (ratio > this.warningThreshold()) return 'var(--nx-color-sensor-green)';
    if (ratio >= this.criticalThreshold()) return 'var(--nx-color-alert-orange)';
  ...
  readonly livesArray = computed(() => Array.from({ length: this.maxLives() }, (_, i) => ({ filled: i < this.lives() })));

  constructor() {
    let previousHintText = this.activeHintText();
    effect(() => {
      const currentHintText = this.activeHintText();
      if (currentHintText && !previousHintText) this.audio.play(SoundEffect.hint);
    ...
    effect(() => {
      ...
      const duration = this.timerDuration();
      if (duration > 0 && st === MinigameStatus.Playing && time > 0 && time <= 5) this.audio.play(SoundEffect.tick);
    });
  }
  ...
  toggleShortcutHints(): void {
    ...
    this._clearAutoHide();
    if (newValue) this._startAutoHide();
  }
  onPauseClick(): void { this.audio.play(SoundEffect.click); this.pauseGame.emit(); }
  onResumeClick(): void { this.audio.play(SoundEffect.click); this.resumeGame.emit(); }

  private _startAutoHide(): void {
    this._autoHideTimer = setTimeout(() => { this.showShortcutHints.set(false); this._autoHideTimer = null; }, MinigameShellComponent.AUTO_HIDE_MS);
  }
  private _clearAutoHide(): void {
    if (this._autoHideTimer !== null) { clearTimeout(this._autoHideTimer); this._autoHideTimer = null; }
  }
}
